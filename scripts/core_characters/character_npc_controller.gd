...
func _update_friendly(delta):
	...
	if nav_agent.is_target_reached():
		var target_position = world.get_random_reachable_point_in_square(
...
func _update_enemy(delta):
	...
	_target_walk_direction_angle += (
		sin((game.time + _sinus_offset) * character.config.walk_direction_sinus_rate)
...
func _update_movement(delta):
	...
	_current_walk_power = move_toward(
		_current_walk_power, _target_walk_power, character.config.walk_power_change_speed * delta
	)

	character.set_walk_power(_current_walk_power)
	character.set_look_direction(tools.angle_to_direction_v2(_current_look_direction_angle))
	character.set_walk_direction(tools.angle_to_direction_v2(_current_walk_direction_angle))
